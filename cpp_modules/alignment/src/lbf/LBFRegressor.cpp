...
void LBFRegressor::Train(const vector<Mat_<uchar> >& images,
    ...
    for (int stage = 0; stage < params_.max_numstage; stage++){
        ...
        cout << "Remaining time is about "<< (t4-t0)/((double)cvGetTickFrequency()*1000*1000*(stage+1))*(params_.max_numstage-stage-1)<< "s"<<endl<<endl;
    }
}
...
vector<Mat_<double> > LBFRegressor::Predict(const vector<Mat_<uchar> >& images,
    ...
    int stage1 =0;
    for ( int stage = 0; stage < params_.max_numstage; stage++){
        if(stage<params_.max_numstage){
            stage1 = stage;
        }
        else{
            stage1 = params_.max_numstage-1;
        }
        struct feature_node ** binfeatures ;
...
void LBFRegressor::Save(const std::string& lbf_path, const string& regressor_path){
    cout << endl<<"Saving model..." << endl;
    ofstream fout;
    fout.open(lbf_path);
    // write the Regressor to file
    WriteGlobalParam(fout);
    WriteRegressor(fout, regressor_path);
    fout.close();
    cout << "End" << endl;
...
void LBFRegressor::Load(const std::string& lbf_path, const string& regressor_path){
    cout << "Loading model from "<< lbf_path  << endl;
    ifstream fin;
    fin.open(lbf_path);
    ReadGlobalParam(fin);
    ReadRegressor(fin, regressor_path);
    fin.close();
    cout << "End"<<endl;
}
...
void  LBFRegressor::WriteRegressor(ofstream& fout, const string& regressor_path){
    for(int i = 0;i < params_.landmark_num;i++){
        fout << mean_shape_(i,0)<<" "<< mean_shape_(i,1)<<" ";
    }
    fout<<endl;
    ofstream fout_reg;
    fout_reg.open(regressor_path,ios::binary);
    for (int i=0; i < params_.max_numstage; i++ ){
...
void LBFRegressor::ReadRegressor(ifstream& fin, const string& regressor_path){
    mean_shape_ = Mat::zeros(params_.landmark_num,2,CV_64FC1);
    for(int i = 0;i < params_.landmark_num;i++){
        fin >> mean_shape_(i,0) >> mean_shape_(i,1);
    }
    ifstream fin_reg;
    fin_reg.open(regressor_path,ios::binary);
    for (int i=0; i < params_.max_numstage; i++ ){
