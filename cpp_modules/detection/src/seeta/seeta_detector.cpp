...
bool SeetaDetector::load_model(const std::vector<std::string>& init_nets,
                               const std::vector<std::string>& predict_nets) {

  assert(predict_nets.size() == 1);
  std::ifstream model_file(predict_nets[0], std::ifstream::binary);
  bool is_loaded = true;
 ...
 bool SeetaDetector::detect(const cv::Mat& src, std::vector<cl::FaceBox>& faces) {
  ...
  // Sliding window
  std::vector<std::vector<cl::FaceBox> > proposals(hierarchy_size_[0]);
      ...
      for (int32_t x = 0; x <= max_x; x += slide_wnd_step_x_) {
        ...
        for (int32_t i = 0; i < hierarchy_size_[0]; i++) {
          if (model_[i]->Classify(&score)) {
            wnd_info.score_ = static_cast<double>(score);
  ...
  std::vector<std::vector<cl::FaceBox> > proposals_nms(hierarchy_size_[0]);
  for (int32_t i = 0; i < hierarchy_size_[0]; i++) {
  ...
  for (int32_t i = 1; i < num_hierarchy_; i++) {
    ...
    for (int32_t j = 0; j < hierarchy_size_[i]; j++) {
      ...
      for (int32_t k = 0; k < num_stage_[cls_idx]; k++) {
        ...
        std::vector<cl::FaceBox> & bboxes = proposals[buf_idx[j]];
        int32_t bbox_idx = 0;
