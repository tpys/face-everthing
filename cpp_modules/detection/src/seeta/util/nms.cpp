...
bool CompareBBox(const cl::FaceBox & a, const cl::FaceBox & b) {
  return a.score_ > b.score_;
}

void NonMaximumSuppression(std::vector<cl::FaceBox>* bboxes,
  std::vector<cl::FaceBox>* bboxes_nms, float iou_thresh) {
  bboxes_nms->clear();
  ...
  while (!all_merged) {
    ...
    for (int32_t i = select_idx; i < num_bbox; i++) {
      ...
      if (static_cast<float>(area_intersect) / area_union > iou_thresh) {
        mask_merged[i] = 1;
        bboxes_nms->back().score_ += (*bboxes)[i].score_;
      }
    }
  }
}
