...
void SeetaDetector::set_param(int min_face_size, int step_x, int step_y, float score_th, float scale_factor)
{
  if (min_face_size >= 20){
    min_face_size_ = min_face_size;
    img_pyramid_.SetMaxScale(wnd_size_ / static_cast<float>(min_face_size));
  }

  if (scale_factor >= 0.01f && scale_factor <= 0.99f)
    img_pyramid_.SetScaleStep(static_cast<float>(scale_factor));
  ...
  if (score_th >= 0)
    cls_thresh_ = score_th;

  SetWindowSize(wnd_size_);
  SetSlideWindowStep(slide_wnd_step_x_, slide_wnd_step_y_);
}


bool SeetaDetector::load_model(const std::string & model_path) {
 ...
 bool SeetaDetector::detect(cv::Mat& src, std::vector<cl::FaceInfo>& faces, bool draw) {
  ...
  int32_t min_img_size = img_data.height <= img_data.width ? img_data.height : img_data.width;
  img_pyramid_.SetImage1x(img_data.data, img_data.width, img_data.height);
  img_pyramid_.SetMinScale(static_cast<float>(wnd_size_) / min_img_size);


  float score;
  cl::FaceInfo wnd_info;
  seeta::Rect wnd;
  float scale_factor = 0.0;
  const seeta::ImageData* img_scaled = img_pyramid_.GetNextScaleImage(&scale_factor);
  wnd.height = wnd.width = wnd_size_;

  // Sliding window
  std::vector<std::vector<cl::FaceInfo> > proposals(hierarchy_size_[0]);
  ...
  while (img_scaled != nullptr) {
    ...
    wnd_info.bbox_.width = static_cast<int32_t>(wnd_size_ / scale_factor + 0.5);
    wnd_info.bbox_.height = wnd_info.bbox_.width;
    ...
    for (int32_t y = 0; y <= max_y; y += slide_wnd_step_y_) {
      ...
      for (int32_t x = 0; x <= max_x; x += slide_wnd_step_x_) {
        ...
        wnd_info.bbox_.x = static_cast<int32_t>(x / scale_factor + 0.5);
        wnd_info.bbox_.y = static_cast<int32_t>(y / scale_factor + 0.5);

        for (int32_t i = 0; i < hierarchy_size_[0]; i++) {
          if (model_[i]->Classify(&score)) {
            wnd_info.face_score_ = static_cast<double>(score);
            proposals[i].push_back(wnd_info);
          }
        }
      }
    }
    img_scaled = img_pyramid_.GetNextScaleImage(&scale_factor);
  }

  std::vector<std::vector<cl::FaceInfo> > proposals_nms(hierarchy_size_[0]);
  for (int32_t i = 0; i < hierarchy_size_[0]; i++) {
  ...
  seeta::ImageData img = img_pyramid_.image1x();
  seeta::Rect roi;
  ...
  for (int32_t i = 1; i < num_hierarchy_; i++) {
    ...
    for (int32_t j = 0; j < hierarchy_size_[i]; j++) {
      ...
      for (int32_t k = 0; k < num_stage_[cls_idx]; k++) {
        ...
        std::vector<cl::FaceInfo> & bboxes = proposals[buf_idx[j]];
        int32_t bbox_idx = 0;

        for (int32_t m = 0; m < num_wnd; m++) {
          if (bboxes[m].bbox_.x + bboxes[m].bbox_.width <= 0 ||
              bboxes[m].bbox_.y + bboxes[m].bbox_.height <= 0)
            continue;
          GetWindowData(img, bboxes[m].bbox_);
          feat_map->Compute(wnd_data_.data(), wnd_size_, wnd_size_);
          feat_map->SetROI(roi);

          if (model_[model_idx]->Classify(&score, mlp_predicts.data())) {
            float x = static_cast<float>(bboxes[m].bbox_.x);
            float y = static_cast<float>(bboxes[m].bbox_.y);
            float w = static_cast<float>(bboxes[m].bbox_.width);
            float h = static_cast<float>(bboxes[m].bbox_.height);

            bboxes[bbox_idx].bbox_.width =
              static_cast<int32_t>((mlp_predicts[3] * 2 - 1) * w + w + 0.5);
            bboxes[bbox_idx].bbox_.height = bboxes[bbox_idx].bbox_.width;
            bboxes[bbox_idx].bbox_.x =
              static_cast<int32_t>((mlp_predicts[1] * 2 - 1) * w + x +
              (w - bboxes[bbox_idx].bbox_.width) * 0.5 + 0.5);
            bboxes[bbox_idx].bbox_.y =
              static_cast<int32_t>((mlp_predicts[2] * 2 - 1) * h + y +
              (h - bboxes[bbox_idx].bbox_.height) * 0.5 + 0.5);
            bboxes[bbox_idx].face_score_ = score;
            bbox_idx++;
          }
        }
  ...
  faces = proposals_nms[0];
  for (int32_t i = 0; i < faces.size(); i++) {
    if (faces[i].face_score_ < cls_thresh_) {
      faces.resize(i);
      break;
    }
  }
    ...
    if(draw)
    {
        for (size_t i = 0; i < faces.size(); i++) {
            cv::rectangle(src, faces[i].bbox_, CV_RGB(0, 0, 255), 4, 8, 0);
        }
    }
...
SeetaDetector::CreateModelReader(seeta::fd::ClassifierType type) {
  std::shared_ptr<seeta::fd::ModelReader> reader;
  switch (type) {
...
SeetaDetector::CreateClassifier(seeta::fd::ClassifierType type) {
  std::shared_ptr<seeta::fd::Classifier> classifier;
  switch (type) {
  case seeta::fd::ClassifierType::LAB_Boosted_Classifier:
...
SeetaDetector::CreateFeatureMap(seeta::fd::ClassifierType type) {
  std::shared_ptr<seeta::fd::FeatureMap> feat_map;
  switch (type) {
  case seeta::fd::ClassifierType::LAB_Boosted_Classifier:
...
void SeetaDetector::GetWindowData(const seeta::ImageData & img,
    const cv::Rect & wnd) {
  int32_t pad_left;
  int32_t pad_right;
  int32_t pad_top;
