...
bool CompareBBox(const cl::FaceInfo & a, const cl::FaceInfo & b) {
  return a.face_score_ > b.face_score_;
}

void NonMaximumSuppression(std::vector<cl::FaceInfo>* bboxes,
  std::vector<cl::FaceInfo>* bboxes_nms, float iou_thresh) {
  bboxes_nms->clear();
  ...
  while (!all_merged) {
    ...
    cv::Rect select_bbox = (*bboxes)[select_idx].bbox_;
    ...
    for (int32_t i = select_idx; i < num_bbox; i++) {
      ...
      if (static_cast<float>(area_intersect) / area_union > iou_thresh) {
        mask_merged[i] = 1;
        bboxes_nms->back().face_score_ += (*bboxes)[i].face_score_;
      }
    }
  }
}
